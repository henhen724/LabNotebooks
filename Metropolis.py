...
class Metropolis:
    def __init__(self, Jnonlocal, Jlocal, Zenergy, Tfinal, Nrepl=500, Nspins=20, steps=10000, sigma=xp.pi/10, sigmaR=0.1, confine_energy_mag=1e12, AnnealT=100., AnnealHigh=1.5):
...
class Metropolis2DWithRadius:
    def __init__(self, Jnonlocal, Jlocal, Tfinal, Nrepl=500, Nspins=20, steps=10000, sigma=xp.pi/10, sigmaR=0.1, confine_energy_mag=1e12, AnnealT=100., AnnealHigh=1.5):
        self.Jnonlocal = check_CuPy(Jnonlocal)
        self.Jlocal = check_CuPy(Jlocal)
        self.Tfinal = Tfinal
        self.Nrepl = Nrepl
        ...
        self.AnnealHigh = AnnealHigh
        self.energy_record = xp.zeros((self.Nrepl, self.steps))
        self.curr_state = xp.zeros((self.Nrepl, self.Nspins, 2))
    ...
    def run(self):
        ...
        displacement = xp.zeros((self.Nrepl, self.Nspins, 2))
        ...
        for i in range(self.steps-1):
            ...
            if xp.random.rand() < 0.5:
                ...
                displacement[:, :, 0] = xp.random.normal(
                    loc=0.0, scale=self.sigmaR, size=(self.Nrepl, self.Nspins))
                displacement[:, :, 1] = xp.zeros((self.Nrepl, self.Nspins))
    ...
    def motional_model(self, this_state):
        energy = xp.zeros(self.Nrepl)
        x_vec = this_state[:, :, 0] * xp.cos(this_state[:, :, 1])
        y_vec = this_state[:, :, 0] * xp.sin(this_state[:, :, 1])
    ...
    def plot_overlap_distribution(self, ax=None):
        ...
        x_vec = self.curr_state[:, :, 0] * xp.cos(self.curr_state[:, :, 1])
        y_vec = self.curr_state[:, :, 0] * xp.sin(self.curr_state[:, :, 1])
        qxx = xp.einsum("ai,bi->ab", x_vec[:], x_vec[:])/self.Nspins
        qyy = xp.einsum("ai,bi->ab", y_vec[:], y_vec[:])/self.Nspins
    ...
    def plot_angle_overlap_distribution(self, ax=None):
        ...
        x_vec = xp.cos(self.curr_state[:, :, 1])
        y_vec = xp.sin(self.curr_state[:, :, 1])
        qxx = xp.einsum("ai,bi->ab", x_vec, x_vec)/self.Nspins
        qyy = xp.einsum("ai,bi->ab", y_vec, y_vec)/self.Nspins
    ...
    def plot_radius_hist(self, ax=None):
        if ax is None:
            fig, ax = plt.subplots()

        if GPUAcc:
            ax.hist(self.curr_state[:,:,0].flatten().get(), bins=20)
        else:
            ax.hist(self.curr_state[:,:,0].flatten(), bins=20)